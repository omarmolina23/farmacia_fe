...
import { getForecastCategory, getPrescriptiveCategory } from "../../../../services/SalesService";
...
export function ForecastByCategory() {
    ...
    useEffect(() => {
        ...
        getForecastCategory(name)
            .then(async (response) => {
                const selCat = categories.find((p) => p.name === selectedCategory);
                const categoryName = selCat?.name || selectedCategory;

                try {
                    const prescriptive = await getPrescriptiveCategory(response, categoryName);
    ...
    return (
        <Card className="bg-[#f0f0f0e7] text-gray-800 border-3 w-full px-1 py-2 overflow-visible relative h-full">
            ...
            <CardContent className="relative grid grid-cols-1 xl:grid-cols-10 gap-6 items-start">
                {loading || !selectedCategory || error ? (
                    <>
